...
class WIoULoss(nn.Module):
    """Wise-IoU v3 (Tong et al. 2023): distance-aware IoU loss with outlier focusing.

    Activated via the YOLO_IoU_LOSS=WIoU environment variable; default CIoU is unchanged.
    """

    def __init__(self, alpha: float = 1.9, delta: float = 3.0, momentum: float = 0.9):
        """Initialize with outlier-focusing hyperparameters and an IoU running mean."""
        super().__init__()
        self.alpha = alpha
        self.delta = delta
    ...
    def forward(
        ...
        """Return the Wise-IoU v3 loss per matched box (outlier reweighted, distance aware)."""
        ...
        distance = ((cx_p - cx_t) ** 2 + (cy_p - cy_t) ** 2) / (cw**2 + ch**2 + 1e-7)
        r_scale = torch.exp(distance)
        l_iou = 1.0 - iou
        with torch.no_grad():
            self.iou_mean = (1 - self.momentum) * self.iou_mean + self.momentum * l_iou.detach().mean()
        beta = l_iou.detach() / (self.iou_mean + 1e-7)
        r_focus = beta / (self.delta * self.alpha ** (beta - self.delta))
        return r_focus * r_scale * l_iou
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16):
        ...
        self.use_wiou = os.environ.get("YOLO_IoU_LOSS", "CIoU").upper() == "WIoU"
        self.wiou = WIoULoss() if self.use_wiou else None
    ...
    def forward(
        ...
        if self.wiou is not None:
            ...
            loss_iou = (self.wiou(pred_bboxes[fg_mask], target_bboxes[fg_mask], iou) * weight).sum() / target_scores_sum
        else:
...
class v8DetectionLoss:
    ...
    def classification_loss(
        ...
        target_scores: torch.Tensor,
        target_scores_sum: torch.Tensor | int,
        dtype: torch.dtype,
    ) -> torch.Tensor:
        """Compute classification loss; specialized detection heads may add auxiliary supervision."""
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        loss[1] = self.classification_loss(preds, pred_scores, target_scores, target_scores_sum, dtype)
...
class HierarchicalDetectionLoss(v8DetectionLoss):
    """Detection loss with auxiliary ship/aircraft/vehicle supervision."""

    def __init__(self, model: torch.nn.Module, tal_topk: int = 10, tal_topk2: int | None = None):
        super().__init__(model, tal_topk, tal_topk2)
        head = model.model[-1]
        self.broad_loss_gain = head.broad_loss_gain
        self.fine_groups = ((0, 1, 2, 3), tuple(range(4, 24)), (24,))
    ...
    def classification_loss(
        ...
        fine_loss = super().classification_loss(preds, pred_scores, target_scores, target_scores_sum, dtype)
        broad_scores = preds["broad_scores"].permute(0, 2, 1).contiguous()
        broad_targets = torch.stack(
            [target_scores[..., list(group)].amax(dim=-1) for group in self.fine_groups], dim=-1
        ).to(dtype)
        broad_loss = self.bce(broad_scores, broad_targets).sum() / target_scores_sum
        return fine_loss + self.broad_loss_gain * broad_loss
