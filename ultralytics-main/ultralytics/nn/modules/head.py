...
class Detect(nn.Module):
    ...
    def forward_head(
        ...
        broad_head: torch.nn.Module = None,
        ...
        if broad_head is not None:
            output["broad_scores"] = torch.cat(
                [broad_head[i](x[i]).view(bs, self.broad_nc, -1) for i in range(self.nl)], dim=-1
            )
        return output
...
class HierarchicalDetect(Detect):
    """YOLO detection head with a shared 3-category auxiliary branch.

    Fine classes keep the official 25-class output. The additional branch predicts ship, aircraft, and vehicle from
    the same feature maps and may be fused into fine-class logits at inference without running another detector.
    """
    ...
    def __init__(
        ...
        nc: int = 25,
        broad_nc: int = 3,
        broad_loss_gain: float = 1.0,
        broad_fusion_gain: float = 0.0,
        reg_max: int = 16,
        end2end: bool = False,
        ch: tuple = (),
    ):
        if nc != 25 or broad_nc != 3:
            raise ValueError("HierarchicalDetect currently requires 25 fine classes and 3 broad categories")
        super().__init__(nc=nc, reg_max=reg_max, end2end=end2end, ch=ch)
        self.broad_nc = broad_nc
        self.broad_loss_gain = float(broad_loss_gain)
        self.broad_fusion_gain = float(broad_fusion_gain)
        c3 = max(ch[0], min(self.nc, 100))
        self.broad_cv3 = nn.ModuleList(
            nn.Sequential(
                nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)),
                nn.Sequential(DWConv(c3, c3, 3), Conv(c3, c3, 1)),
                nn.Conv2d(c3, self.broad_nc, 1),
            )
            for x in ch
        )
        if end2end:
            self.one2one_broad_cv3 = copy.deepcopy(self.broad_cv3)
        self.register_buffer(
            "fine_to_broad",
            torch.tensor([0, 0, 0, 0] + [1] * 20 + [2], dtype=torch.long),
            persistent=True,
        )

    @property
    def one2many(self):
        return dict(box_head=self.cv2, cls_head=self.cv3, broad_head=self.broad_cv3)
    ...
    def one2one(self):
        return dict(
            ...
            broad_head=self.one2one_broad_cv3,
        )

    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        dbox = self._get_decode_boxes(x)
        fine_logits = x["scores"]
        if self.broad_fusion_gain:
            broad_logits = x["broad_scores"].index_select(1, self.fine_to_broad)
            fine_logits = fine_logits + self.broad_fusion_gain * broad_logits
        return torch.cat((dbox, fine_logits.sigmoid()), 1)

    def bias_init(self):
        super().bias_init()
        for i, head in enumerate(self.broad_cv3):
            head[-1].bias.data[: self.broad_nc] = math.log(
                5 / self.broad_nc / (640 / self.stride[i]) ** 2
            )
        if self.end2end:
            for i, head in enumerate(self.one2one_broad_cv3):
                head[-1].bias.data[: self.broad_nc] = math.log(
                    5 / self.broad_nc / (640 / self.stride[i]) ** 2
                )
