...
class CoordinateAttention(nn.Module):
    """Coordinate Attention (Hou et al., CVPR 2021): spatial-coordinate aware channel attention.

    Suitable for remote-sensing small objects where global pooling loses spatial detail.
    """

    def __init__(self, inp: int, oup: int | None = None, reduction: int = 32) -> None:
        super().__init__()
        if oup is None:
            oup = inp
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply coordinate attention: factorized horizontal and vertical attention maps."""
        identity = x
        n, c, h, w = x.shape
        x_h = x.mean(dim=3, keepdim=True)  # (n, c, h, 1)
        x_w = x.mean(dim=2, keepdim=True).permute(0, 1, 3, 2)  # (n, c, 1, w)
