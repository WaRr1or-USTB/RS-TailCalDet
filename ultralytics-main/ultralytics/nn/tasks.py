...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        loss_fn = HierarchicalDetectionLoss if isinstance(self.model[-1], HierarchicalDetect) else v8DetectionLoss
        return E2ELoss(self, loss_fn=loss_fn) if getattr(self, "end2end", False) else loss_fn(self)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [ch[f]]
        elif m in frozenset({CBAM, ECA, CoordinateAttention}):
            args = [ch[f], *args]
            c2 = ch[f]
        elif m is Concat:
